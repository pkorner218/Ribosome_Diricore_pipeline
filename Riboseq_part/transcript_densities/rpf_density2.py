...
if Species == "human":
	REFfile = open("/DATA/pkoerner/packages/Ribopipe/auto/transcript_densities/human_CDS_UTRS.fa","r")

if Species == "mouse":
	REFfile = open("/DATA/pkoerner/packages/Ribopipe/auto/transcript_densities/mouse_CDS_UTRS.fa","r")
...
if int(len(sys.argv)) > 3:
	...
	projectname = projectfolder.split("/output/transcript/QC")[0]
	print(projectname)
	projectname = projectname.rsplit("/",2)[1]
...
#			print(ENSline)
				Reads = ENSline[1] + "\t" + ENSline[3]
#			print(Reads)

#			master_dict[filename][ENST]["Gene"].append(Gene)

#			if "UTR5:" in REFdict[ENST][1]:
#				print(REFdict[ENST])
#				print(REFdict[ENST][1])

#			if "UTR5:" in line:

#				if "UTR3:" in REFdict[ENST][3]:
...
#					Reads = line[8]	
			...
			#read_median = read_x1 + 11
				...
				if "e" not in ENSline[2]:
					read_median = int(ENSline[2])
				else:
					read_median = int(9999)
...
savename = str(projectpre + 'global_rpf_density.pdf')
