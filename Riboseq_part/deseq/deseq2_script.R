...
sampleCondition<-c('mIFN', 'mIFN', 'IFN', 'IFN')
sampleType<-c('mIFN', 'mIFN', 'IFN', 'IFN')
...
write.table(normalized_counts, file="test_normalized_counts.txt", sep="\t", quote=F, col.names=NA)
...
pdf(file="test_barplotTranscriptReadsPerSample.pdf", width=7, height=5)
...
pdf(file="test_dendrogramCorrelationHclust.pdf", width=7, height=5, bg="transparent")
...
colData(ddsHTSeq)$condition<-factor(colData(ddsHTSeq)$condition, levels=c("IFN","mIFN"))
...
#pdf(file="test_MA.pdf", width=7, height=5)
#par(font.lab=2, mar=c(12, 5.5, 2, 2))
...
pdf(file="test_res_MA.pdf", width=7, height=5)
par(font.lab=2, mar=c(12, 5.5, 2, 2))
...
pdf(file="test_PCA.pdf", width=7, height=5)
par(font.lab=2, mar=c(12, 5.5, 2, 2))

print(plotPCA(rld, intgroup=c('condition','type')))
#dev.copy(png,"test_pca.png")
dev.off()
...
pdf(file="test_deseq2_PCA.pdf", width=7, height=5)
par(font.lab=2, mar=c(12, 5.5, 2, 2))
...
head(pca$rotation)
head(pca$x)
write.table(as.data.frame(pca$rotation),file="test_pca_geneloadingsx.txt",quote=F,sep="\t")
#quit()
...
DEcellresults <- results(DEseqresult, contrast=c("condition","IFN","mIFN"))
summary(DEcellresults)
write.table(as.data.frame(DEcellresults),file="test_DESEQ2.txt",quote=F,sep="\t")
...
df <- read.table("test_DESEQ2.txt", header = TRUE)

pdf(file="testbasic_volcano.pdf", width=7, height=5, bg="transparent")
par(mar=c(8, 4, 2, 2), font.lab=2)
